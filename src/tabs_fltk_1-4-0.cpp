// Richard Spinney 2021:
// Including Fl_Tabs widget from FLTK v1.4.x as it has protected members (not private)
// As such we can subclass it and have access to low level drawing, thus changing the color of the tabs etc.
// Other that the change to the name of the class, include of accompanying header + one line edit to avoid compiler warnings, 
// it is the same file with the same attribution.
...
#include "./include/tabs_fltk_1-4-0.hpp"
...
int fl_tabs_1_4_0::tab_positions() {
...
*/
Fl_Widget *fl_tabs_1_4_0::which(int event_x, int event_y) {
...
void fl_tabs_1_4_0::redraw_tabs()
{
  int H = tab_height();
  if (H >= 0) {
    H += Fl::box_dy(box());
...
int fl_tabs_1_4_0::handle(int event) {
  ...
  case FL_SHOW:
    ...
    return Fl_Group::handle(event); //2021 <---- avoid fallthrough warning
  default:
    return Fl_Group::handle(event);
...
/**
  ...
  \see push().
*/
int fl_tabs_1_4_0::push(Fl_Widget *o) {
  if (push_ == o) return 0;
...
*/
int fl_tabs_1_4_0::value(Fl_Widget *newvalue) {
  Fl_Widget*const* a = array();
  int ret = 0;
  for (int i=children(); i--;) {
    Fl_Widget* o = *a++;
...
void fl_tabs_1_4_0::draw_tab(int x1, int x2, int W, int H, Fl_Widget* o, int what) {
...
*/
fl_tabs_1_4_0::fl_tabs_1_4_0(int X, int Y, int W, int H, const char *L) :
  Fl_Group(X,Y,W,H,L)
{
  box(FL_THIN_UP_BOX);
  push_ = 0;
...
/**
  ...
  \since        FLTK 1.3.0
*/
void fl_tabs_1_4_0::client_area(int &rx, int &ry, int &rw, int &rh, int tabh) {
...
void fl_tabs_1_4_0::clear_tab_positions() {
  if (tab_pos) {
    free(tab_pos);
    tab_pos = 0;
  }
